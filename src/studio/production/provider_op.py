...
INITIATED →(提交前取消)→ ABORTED(墓碑;迟到的 Claim/Submit 被拒)

- webhook 按 provider_event_id 去重;同终态异 result/cost -> IdempotencyConflict。
- SUBMISSION_UNKNOWN 非终态:ReconciliationConfirmed* 可恢复到 SUBMITTED/SUCCEEDED/FAILED。
"""
...
class ProviderOperationDecider:
    ...
    def _initiate(
        ...
        expected = identity.operation_id(
            cmd.spec.attempt_id, cmd.spec.logical_operation_key
        )
        if cmd.operation_id != expected:
            return Rejected("forged_operation_id", "operation_id 与 spec 派生不一致")
        if state.status is not None:
            return Accepted(())  # 幂等:已存在
        return Accepted(
            (
                ProposedEvent(
                    "initiated",
    ...
    def _abort(
        ...
        if state.status is ProviderOpStatus.ABORTED:
            return Accepted(())
        if state.status not in (ProviderOpStatus.INITIATED, ProviderOpStatus.CLAIMED):
    ...
    def _dedup(self, state: ProviderOpState, provider_event_id: str) -> bool:
        return provider_event_id in state.seen_events
    ...
    def _succeeded(
        ...
        allowed = (ProviderOpStatus.SUBMITTED, ProviderOpStatus.SUBMISSION_UNKNOWN)
        if state.status not in allowed:
    ...
    def evolve(self, state: ProviderOpState, event: ProviderOpEvent) -> ProviderOpState:
        if isinstance(event, ProviderOperationInitiatedEvt):
            return state.model_copy(update={"status": ProviderOpStatus.INITIATED})
        ...
        if isinstance(event, ProviderOperationSubmittedEvt):
            return state.model_copy(
                update={
                    ...
                    "job_id": event.job_id,
                    "seen_events": (*state.seen_events, event.provider_event_id),
                }
            )
        ...
        if isinstance(event, ProviderOperationAbortedEvt):
            return state.model_copy(update={"status": ProviderOpStatus.ABORTED})
        return state
